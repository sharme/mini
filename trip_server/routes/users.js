...
router.get('/getUserbyUID', function (req, res, next) {
  ...
  var SQL = mysql.format("select u_id, u_name, u_avatar, u_gender, u_city, u_level, u_created_time," +
      ...
      "(select count(t_id) from likes as ll where ll.u_id = ur.u_id) as l_count," +
      "(select count(n_id) from news as nw where nw.n_updated_by IS NULL and nw.n_to = ur.u_id) as n_count from users as ur where ur.u_id = ?", [req.param('u_id')]);
